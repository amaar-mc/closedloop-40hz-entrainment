---
source_file: "validation/experiments/fatigue_model_sensitivity.py"
type: "code"
community: "Control Strategies & Validation"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Control_Strategies_&_Validation
---

# fatigue_model_sensitivity.py

## Connections
- [[ExponentialDecaySimulator]] - `contains` [EXTRACTED]
- [[Fatigue Model Sensitivity Analysis for Closed-Loop 40Hz Entrainment  Addresses]] - `rationale_for` [EXTRACTED]
- [[FatigueModelConfig]] - `contains` [EXTRACTED]
- [[HeterogeneousPopulationSimulator]] - `contains` [EXTRACTED]
- [[PairwiseResult]] - `contains` [EXTRACTED]
- [[SaturationModelSimulator]] - `contains` [EXTRACTED]
- [[StepFunctionSimulator]] - `contains` [EXTRACTED]
- [[create_simulator()]] - `contains` [EXTRACTED]
- [[main()_69]] - `contains` [EXTRACTED]
- [[print_robustness_verdict()]] - `contains` [EXTRACTED]
- [[print_summary_table()_1]] - `contains` [EXTRACTED]
- [[rigorous_validation.py]] - `imports_from` [EXTRACTED]
- [[run_fatigue_model_experiment()]] - `contains` [EXTRACTED]
- [[run_single_trial()_1]] - `contains` [EXTRACTED]
- [[simulator.py]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Control_Strategies_&_Validation