---
source_file: "temporal_multiscale/run_comparison_study.py"
type: "code"
community: "Community 65"
location: "L1"
tags:
  - community/Community_65
---

# run_comparison_study.py

## Connections
- [[Architecture comparison study train and evaluate all models across horizons.  R]] - `rationale_for` [EXTRACTED]
- [[_dataset_base_4ch()]] - `contains` [EXTRACTED]
- [[_dataset_base_7ch()]] - `contains` [EXTRACTED]
- [[_dataset_dir()]] - `contains` [EXTRACTED]
- [[_device()]] - `contains` [EXTRACTED]
- [[_ensure_dataset()]] - `contains` [EXTRACTED]
- [[_n_features()]] - `contains` [EXTRACTED]
- [[_print_table()]] - `contains` [EXTRACTED]
- [[_run_lstm()]] - `contains` [EXTRACTED]
- [[_run_persistence()]] - `contains` [EXTRACTED]
- [[_run_ridge()]] - `contains` [EXTRACTED]
- [[_run_tcn()]] - `contains` [EXTRACTED]
- [[_run_transformer()]] - `contains` [EXTRACTED]
- [[_run_xgboost()]] - `contains` [EXTRACTED]
- [[main()_22]] - `contains` [EXTRACTED]
- [[parse_args()_3]] - `contains` [EXTRACTED]

  #community/Community_65