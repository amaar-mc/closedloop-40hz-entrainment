---
source_file: "results/RESULTS_REPORT.md"
type: "document"
community: "Controller Results & Judge Prep"
location: "## 2. Controller Comparison"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Controller_Results_&_Judge_Prep
---

# Controller Comparison Table (Fixed/Reactive/TCN/Hybrid/Oracle, N=35)

## Connections
- [[Alignment Oracle Controller (theoretical upper bound)]] - `references` [EXTRACTED]
- [[CSEF Results Slide R²=0.287 static ceiling, horizon sweep inflection at 3s, controller comparison chart N=35]] - `shares_data_with` [EXTRACTED]
- [[Controller Comparison Table N=35 TCN 72.1% alignment, 82.6% low-PAC targeting, Oracle 91%]] - `shares_data_with` [EXTRACTED]
- [[Epoch Alignment Metric (Low-PAC Stim Rate + High-PAC Rest Rate  2)]] - `references` [EXTRACTED]
- [[Fatigue Sensitivity Analysis (Simulation, 6 habituation levels)]] - `references` [EXTRACTED]
- [[Figure 8 Controller Comparison Bar Chart — Fixed 45%, Reactive 64%, PI 66%, TCN 72%, Hybrid 74%, Oracle 100% alignment]] - `shares_data_with` [EXTRACTED]
- [[MultiscaleCausalTCN Model Performance (73-feature, 31K params)]] - `references` [EXTRACTED]
- [[PAC Targeting Gap Metric (PAC during rest minus PAC during stim)]] - `references` [EXTRACTED]
- [[TCN Predictive vs Reactive Threshold Primary Result]] - `references` [EXTRACTED]
- [[TCN vs Reactive Primary Statistics alignment g=1.31, low-PAC targeting g=4.47, PAC gap g=1.57, all p0.001]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Controller_Results_&_Judge_Prep