---
source_file: "tests/test_streaming_parity.py"
type: "code"
community: "Models, Streaming & Apps"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Models,_Streaming_&_Apps
---

# test_streaming_parity.py

## Connections
- [[Parity verification StreamingFeatureExtractor vs offline extract_spectral_featu]] - `rationale_for` [EXTRACTED]
- [[_feature_slices()]] - `contains` [EXTRACTED]
- [[_make_windows()]] - `contains` [EXTRACTED]
- [[main()_20]] - `contains` [EXTRACTED]
- [[srcstreamingfeature_extractor.py (StreamingFeatureExtractor)]] - `imports_from` [EXTRACTED]
- [[test_filter_state_persists()]] - `contains` [EXTRACTED]
- [[test_n_features_property()]] - `contains` [EXTRACTED]
- [[test_no_filtfilt_in_streaming()]] - `contains` [EXTRACTED]
- [[test_output_shape_4ch()]] - `contains` [EXTRACTED]
- [[test_output_shape_7ch()]] - `contains` [EXTRACTED]
- [[test_parity_4ch()]] - `contains` [EXTRACTED]
- [[test_parity_7ch()]] - `contains` [EXTRACTED]
- [[test_reset_zeroes_state()]] - `contains` [EXTRACTED]
- [[test_wrong_shape_raises()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Models,_Streaming_&_Apps