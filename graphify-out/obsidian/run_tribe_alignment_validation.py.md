---
source_file: "scripts/pipeline/run_tribe_alignment_validation.py"
type: "code"
community: "12-Feature TCN Validation"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/12-Feature_TCN_Validation
---

# run_tribe_alignment_validation.py

## Connections
- [[FixedScheduleCtrl]] - `contains` [EXTRACTED]
- [[OracleCtrl]] - `contains` [EXTRACTED]
- [[PredictiveCtrl]] - `contains` [EXTRACTED]
- [[ReactiveCtrl]] - `contains` [EXTRACTED]
- [[TRIBE V2-Enhanced Simulation Alignment & Clinical Utility Evaluation  Computes]] - `rationale_for` [EXTRACTED]
- [[evaluate_alignment()]] - `contains` [EXTRACTED]
- [[hedges_g()]] - `contains` [EXTRACTED]
- [[main()_38]] - `contains` [EXTRACTED]
- [[run_subject_trial()]] - `contains` [EXTRACTED]
- [[simulator.py]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/12-Feature_TCN_Validation