---
source_file: "submission/CSEF_vs_Synopsys_Rules.md"
type: "document"
community: "Community 69"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_69
---

# CSEF vs Synopsys Rules Comparison Document

## Connections
- [[13-Page Project Presentation PDF (CSEF Requirement)]] - `references` [EXTRACTED]
- [[AI  Generative AI Use Policy (ISEF-based)]] - `references` [EXTRACTED]
- [[Awards Structure and ISEF Advancement Paths]] - `references` [EXTRACTED]
- [[CSEF Action Items and Preparation Checklist]] - `references` [EXTRACTED]
- [[CSEF Consensus-Based Judging Process]] - `references` [EXTRACTED]
- [[California State Science Fair (CSEF)]] - `references` [EXTRACTED]
- [[Conduct and Disqualification Rules]] - `references` [EXTRACTED]
- [[Display Board Dimension and Content Rules]] - `references` [EXTRACTED]
- [[Fair Schedule and Logistics Comparison]] - `references` [EXTRACTED]
- [[Project Notebook  Lab Notebook Requirements]] - `references` [EXTRACTED]
- [[Required Forms and Paperwork Comparison]] - `references` [EXTRACTED]
- [[Synopsys 40-Point Numerical Judging Rubric]] - `references` [EXTRACTED]
- [[Synopsys Silicon Valley Science & Technology Championship (SCVSEF)]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_69