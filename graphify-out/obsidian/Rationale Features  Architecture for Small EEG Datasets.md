---
source_file: "archive/docs_v1_v8/FINAL_ASSESSMENT.md"
type: "document"
community: "Model Ceiling & Leakage Audits"
tags:
  - community/Model_Ceiling_&_Leakage_Audits
---

# Rationale: Features > Architecture for Small EEG Datasets

## Connections

- [[Honest Baseline Ridge Regression R²=0.287 (135 spectral+wavelet features)]] - `references` [EXTRACTED]
- [[Rationale PAC is Theta-Gamma Frequency Coupling — Explicit Features Required]] - `rationale_for` [INFERRED]

  #community/Model*Ceiling*&\_Leakage_Audits
