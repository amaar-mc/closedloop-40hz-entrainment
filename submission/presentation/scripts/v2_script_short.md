...
The model uses only 12 features that track PAC trajectory and stimulation context -- I discovered that the 61 spectral EEG features I originally included were encoding subject-specific brain anatomy that doesn't generalize across patients. Dropping them raised test R-squared from 0.12 to 0.60 -- a five-fold improvement.

*[Point to Figure 5: Horizon Sweep]*

The reason 5 seconds matters is shown in this chart. I tested how well different methods predict brain state at different time horizons. At 1 to 2 seconds ahead, simple approaches work fine -- the brain barely changes. But at 5 seconds, every simple method fails completely. My model is the only one that still gives useful predictions at that range -- R-squared of 0.60 -- and that's exactly the lead time a controller needs to actually make a difference.
...
I've deployed a live caregiver app at huggingface.co/spaces/amaarc/neurocare-40hz where you can see the system running in real time.
...
> And the biggest discovery came later: I realized that the 61 spectral features I was using were causing catastrophic overfitting -- validation R-squared was 0.33 but test was negative 0.03. When I dropped those features and kept only the 12 that track PAC trajectory and stimulation context, test R-squared jumped to 0.60. The spectral features were the bottleneck all along.
...
| **Design & Methodology** (15 pts) | Explored 8 architectures (expansion module); identified the data ceiling; pivoted to temporal approach; discovered spectral feature overfitting |
...
| **Creativity** (20 pts) | "Nobody was trying to predict ahead of time"; the architecture marathon as a method to prove a ceiling; the horizon sweep concept; PAC+Stim feature discovery |
...
| How deeply do you understand it? | Explains PAC intuitively, knows why 0.60 is useful, can describe causal guarantees, can explain why spectral features fail |
| Did you discover errors? | SpecTempNet leakage story (expansion module); spectral feature overfitting discovery |
| Do conclusions follow from data? | "83% vs 52%" tied directly to the replay; "all 35 out of 35" |
| How new or different is this? | "Not just reacting... anticipating the drop before it happens"; feature discovery narrative |
