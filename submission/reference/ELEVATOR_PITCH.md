...
I built a causal temporal convolutional network that predicts neural state 5 to 10 seconds
ahead — R-squared of 0.60 where simpler baselines fail — using just 12 PAC trajectory
and stimulation context features, a five-fold improvement over my original model. The system
uses that forecast to stimulate proactively, when it is most effective.
...
Scan the QR code to try the live caregiver app at huggingface.co/spaces/amaarc/neurocare-40hz.
The next step is a formal pilot at a memory care facility to collect real caregiver feedback.
