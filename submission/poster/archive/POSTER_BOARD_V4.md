# POSTER BOARD CONTENT — V4 (Print-Ready, Audit-Corrected)
...
|  |  Adaptive Concept|  |  |   METHODS & PROCEDURES     |  |  +------------------+  |
|  |                  |  |  |                            |  |                        |
|  +------------------+  |  |  [FIG 2] System Arch.      |  |  +------------------+  |
|                        |  |                            |  |  |   CONCLUSIONS /  |  |
|  +------------------+  |  +----------------------------+  |  | FURTHER RESEARCH |  |
...
|  |                  |  |  |    DATA / CHARTS           |  |                        |
|  +------------------+  |  |                            |  |  +------------------+  |
|                        |  |  [FIG 3] Horizon Sweep     |  |  |   REFERENCES /   |  |
|                        |  |   (central finding)        |  |  | ACKNOWLEDGEMENTS |  |
|                        |  |                            |  |  +------------------+  |
|                        |  |  [FIG 4] Timeline          |  |                        |
|                        |  |   Example (real data)      |  |                        |
|                        |  |                            |  |                        |
...
_Two-panel schematic diagram._

_Left panel — "Fixed Schedule (Current)":_

- _Top: flat line labeled "Stimulation" showing uniform 40s ON / 20s OFF blocks_
- _Bottom: fluctuating PAC signal showing periods where stimulation continues during high PAC (wasted) and stops during low PAC (missed)_
- _Red X marks on misaligned regions_

_Right panel — "Adaptive Schedule (This Project)":_

- _Top: variable-length ON/OFF blocks that track the PAC signal_
- _Bottom: same PAC signal, but stimulation aligns with low-PAC periods_
- _Green checkmarks on aligned regions_
- _Arrow from PAC signal to controller decision labeled "TCN predicts 5-10s ahead"_
...
| **Key feature**  | Real-time inference               | 31-step receptive field (covers full 20-second lookback window), causal (no future leakage) |

---

### ARCHITECTURE EXPLORATION (48-72pt header, 24pt+ body)

Before building the temporal predictor, I tested 8 different neural network architectures for static PAC prediction — estimating current PAC from a single 2-second EEG window. The architectures spanned three orders of magnitude in model size:
...
The simplest models (EEGNet, Ridge, EEGNetLarge) all converge to R² = 0.287, while larger or more complex architectures (SpecTempNet, ViT-TCNet, ATCNet) perform _worse_ due to overfitting. This confirms a fundamental data ceiling set by epoch-level PAC labels on 2-second windows, not a model capacity limitation. The largest model (2 million parameters) performed worst among deep learning approaches due to overfitting on the limited training data.

This finding reframed the problem: instead of predicting PAC better from single windows, predict it further into the future using sequences. That insight led to the causal TCN, which operates on 20-second temporal histories rather than isolated 2-second snapshots.
...
_Flowchart showing the closed-loop pipeline:_

1. _Patient icon with EEG headset ->_
2. _"Raw EEG (7 frontal channels)" box ->_
3. _"EEGNet (1,457 params)" box -> "Current PAC Estimate" ->_
4. _"Feature Extraction (73 features)" box ->_
5. _"Causal TCN (31K params)" box -> "Predicted PAC (5s ahead)" ->_
6. _"Closed-Loop Controller" decision diamond -> three outputs: STIMULATE / REST / MAINTAIN_
7. _Arrow loops back from controller output to patient (audio speaker icon)_
8. _Label inference time: "<50ms total"_
...
_Source: Generate from data points below (line graph)._

_X-axis: Prediction Horizon (1, 2, 3, 5, 8, 10 seconds)_
_Y-axis: R² (-0.4 to 0.9)_
_Three lines:_

- _Persistence baseline (blue): 0.760, 0.488, 0.234, -0.267, -0.276, -0.256_
- _Ridge regression (orange): 0.812, 0.542, 0.254, -0.393, -0.211, -0.212_
- _Causal TCN (green): 0.735, 0.470, 0.277, 0.254, 0.240, 0.278_

_Shading: light red below R²=0 ("Worse than mean prediction"), light green for 5-10s zone ("Controller-relevant horizon"). Double-headed arrow showing "+0.5 R² margin" in the 5-10s zone._

**Interpretation:** At 1-2 second horizons, simple baselines (just repeating the last value) work fine. But at 5-10 seconds, the lead time a controller needs to act proactively, all baselines collapse below zero while the TCN maintains R² approximately 0.25. This +0.5 margin is what makes predictive control possible.

**[FIGURE 4: Real-Data Controller Timeline — `results/figures/timeline_example.png`]**

_Existing figure. Shows a real test subject (sub-15) with:_

- _Top panel: actual PAC signal with low-PAC (orange) and high-PAC (blue) epochs marked_
- _Middle panel: Reactive controller decisions (green = stimulate, gray = rest)_
- _Bottom panel: TCN controller decisions (orange = stimulate, gray = rest)_
- _Visual comparison: TCN stimulates during more low-PAC periods than Reactive_
...
TCN vs Reactive: Alignment g = +1.31, Low-PAC targeting g = +4.47, PAC gap g = +1.57 (all p < 0.001, Wilcoxon signed-rank). The TCN's PAC targeting gap reaches **91% of the theoretical oracle bound** (30.5 vs 33.3).

**Trade-off:** The TCN uses more stimulation (59.7%) than reactive (36.7%) because it proactively pre-positions stimuli before PAC declines. However, it still uses 10% less stimulation than the original fixed-schedule protocol (66.6%), while achieving far superior targeting. The reactive controller achieves higher high-PAC rest rate (77.3% vs 61.6%) by being more conservative, but at the cost of missing 48% of low-PAC windows that need stimulation.

**[FIGURE 5: Controller Comparison — `results/figures/controller_comparison.png`]**

_Existing figure. Grouped bar chart showing 6 controllers x 3 metrics (Alignment, Low-PAC Stim Rate, High-PAC Rest Rate) with error bars and significance brackets (g = 1.31, g = 4.47)._
...
_Existing figure. Scatter plot of TCN utility (y-axis) vs Reactive utility (x-axis) for all 35 subjects. Every point falls above the y = x diagonal, indicating every subject benefits from TCN control. Points colored by split (Train/Val/Test) to show generalization._
...
1. At 5-10 second prediction horizons, the causal TCN maintains R² approximately 0.25 while all baselines collapse below zero, a +0.5 R² margin at the operationally relevant range for proactive neural stimulation control
...
| Materials                                | ~120           |
| Architecture Exploration                 | ~120           |
| Methods & Procedures                     | ~430           |
| Results text (all 4 results + trade-off) | ~290           |
| Data Integrity                           | ~45            |
| Conclusions                              | ~135           |
| Further Research                         | ~50            |
| Broader Impact                           | ~45            |
| **Total body text**                      | **~1,720**     |

This is appropriate for a 48" x 56" board at 24pt font. Tables and figures will occupy approximately 50% of the board area.
...
1. **CRITICAL FIX: ATCNet R² corrected from 0.287 to 0.075** — All documentation (lab notebook, comprehensive analysis report, CODE_MAP.md) confirms ATCNet achieved R² = 0.075, not 0.287. The V3 value was incorrect.

2. **Architecture Exploration text revised** — Changed "All converge to R² = 0.287" to accurately reflect that the simplest models converge to R² = 0.287 while more complex architectures perform _worse_ due to overfitting. This is a stronger narrative: it shows that model complexity hurts, not just that there's a ceiling.

3. **PAC Gap units corrected** — Changed from "µV²" to "(x10^-6)" since the Modulation Index (Tort 2010) is a dimensionless KL-divergence measure, not in voltage-squared units. A knowledgeable judge would catch incorrect units.

4. **Stim % column added to controller comparison table** — Shows the stimulation budget each controller uses (Fixed 66.6%, Reactive 36.7%, TCN 59.7%, Oracle 48.3%). Judges will ask about this; including it proactively shows transparency.

5. **Trade-off paragraph added after Result 1** — Explicitly acknowledges that TCN uses more stimulation than Reactive (59.7% vs 36.7%) and that Reactive wins on High-PAC Rest Rate (77.3% vs 61.6%), but contextualizes this: TCN still uses less stim than Fixed Schedule while achieving far better targeting. This prevents the appearance of cherry-picking metrics.

6. **"91% of oracle" clarified** — Now explicitly states this applies to PAC targeting gap (30.5 vs 33.3), not to alignment or low-PAC targeting where the ratio is lower.

_Version 4 — Audit-corrected from V3. All numerical claims verified against results/RESULTS_REPORT.md and git commit history._
