# POSTER BOARD CONTENT — V3 (Print-Ready)
...
All converge to R² = 0.287, confirming a fundamental data ceiling (epoch-level PAC labels on 2-second windows), not a model capacity limitation. The largest model (2 million parameters) actually performed _worse_ than the smallest (1,457 parameters) due to overfitting on the limited training data.
...
| Further Research               | ~50            |
| Broader Impact                 | ~45            |
| **Total body text**            | **~1,680**     |

This is appropriate for a 48" x 56" board at 24pt font. The Architecture Exploration section adds approximately 120 words over V2 but provides crucial narrative context for the transition from static to temporal prediction. Tables and figures will occupy approximately 50% of the board area.
...
_Version 3 — Updated from V2: added Architecture Exploration section with 6-architecture comparison table; corrected TCN receptive field to 31-step; corrected TCN loss description to single-task Huber; updated word count estimate._
