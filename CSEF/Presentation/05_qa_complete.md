...
**What are the 12 PAC+Stim input features?**
...
The previous model used 73 features (61 spectral + 7 PAC-derived + 5 stim context). The spectral features encoded subject-specific brain anatomy (skull thickness, electrode impedance, oscillation profiles) that doesn't generalize across patients. Dropping them raised test R-squared from 0.121 to 0.606 — a five-fold improvement.
...
**R-squared of 0.60 at 5 seconds — how does that translate to clinical benefit?**

The controller doesn't need the exact PAC value — it needs directional accuracy. Is coupling trending down or holding steady? R-squared 0.60 is more than enough for the controller to target 82.6% of low-PAC windows versus 51.7% for reactive. The proof is in the downstream result: 72% alignment, 83% therapeutic targeting, all 35 patients benefit. The controller amplifies even modest prediction accuracy into meaningful therapeutic improvement — and at 0.60, the prediction is far from modest.

For context: the previous 73-feature model achieved only R-squared 0.121 on test subjects. The PAC+Stim 12-feature model at 0.606 represents a five-fold improvement, validated across 5 random seeds (mean 0.606 ± 0.032, range 0.558–0.647).
