"""
Replay real per-window PAC data through controllers — no simulator, no fake dynamics.

Identical analysis to run_replay_analysis.py but using per-window PAC labels
(recomputed with 9 phase bins) instead of epoch-level PAC. After the main analysis,
loads epoch-level results and prints a side-by-side delta table.
...
Usage:
    venv/Scripts/python.exe perwindow_pac/run_replay_analysis.py
"""
...
class CUSUMControl:
    ...
    def step(self, pac: float) -> int:
        ...
        if self.S > h:
            self.S = 0.0
            self.stim_remaining = self.stim_dur - 1
...
# ---------------------------------------------------------------------------
# Data loading — uses perwindow_pac data
# ---------------------------------------------------------------------------

def load_subjects() -> List[Dict]:
    """Load all subjects with per-window PAC, raw EEG windows, + stim/rest labels."""
    processed_dir = ROOT / "data" / "processed" / "perwindow_pac"
    raw_root = ROOT / "data" / "raw" / "ds005048"

    all_subjects = []

    for split in ["train", "val", "test"]:
        npz_path = processed_dir / f"{split}_data.npz"
        if not npz_path.exists():
            print(f"  WARNING: {npz_path} not found, skipping {split}")
            continue
...
def precompute_biomarker_features(subjects: List[Dict]) -> Dict[str, Dict[str, np.ndarray]]:
    ...
    cache_dir = ROOT / "data" / "processed" / "perwindow_pac" / "biomarker_cache"
...
# ---------------------------------------------------------------------------
# Comparison with epoch-level results
# ---------------------------------------------------------------------------

def print_comparison(perwindow_summary: Dict, epoch_results_path: Path):
    """Print side-by-side delta table vs epoch-level results."""
    if not epoch_results_path.exists():
        print("\n  (Epoch-level results not found — skipping comparison)")
        return

    with open(epoch_results_path) as f:
        epoch_data = json.load(f)

    epoch_summary = epoch_data.get("summary", {})

    print(f"\n{'='*95}")
    print("COMPARISON: Per-Window PAC vs Epoch-Level PAC")
    print(f"{'='*95}")
    print(
        f"{'Controller':<28s} "
        f"{'Metric':<16s} "
    ...
    metrics_to_compare = [
        ("stim_pct", "Stim%", "%"),
        ("stim_hit_rate_pct", "Hit Rate%", "%"),
        ("wasted_stim_pct", "Wasted%", "%"),
    ]

    for name in ctrl_order:
        e = epoch_summary.get(name, {})
        p = perwindow_summary.get(name, {})
        if not e or not p:
            continue

        for metric_key, label, suffix in metrics_to_compare:
            e_val = e.get(metric_key, 0)
            p_val = p.get(metric_key, 0)
            delta = p_val - e_val
            print(
                f"{name:<28s} "
                f"{label:<16s} "
...
def main():
    t0 = time_mod.time()
    print("=" * 90)
    print("REPLAY ANALYSIS: Per-Window PAC Data Through Each Controller")
    print("=" * 90)

    print("\nLoading per-window PAC subject data...")
    subjects = load_subjects()
    print(f"Loaded {len(subjects)} subjects")

    if not subjects:
        print("ERROR: No subjects loaded. Run recompute_pac.py first.")
        return

    print("Extracting biomarker features...")
    all_biomarkers = precompute_biomarker_features(subjects)

    # Free raw EEG windows from memory
    for subj_data in subjects:
    ...
    # ------------------------------------------------------------------
    print("=" * 90)
    print("RESULTS: Per-Window PAC Replay")
    print("=" * 90)
    print(
        f"{'Controller':<28s} "
        f"{'Stim%':>6s} "
    ...
    print(f"\n{'='*90}")
    print("KEY FINDINGS (Per-Window PAC)")
    print(f"{'='*90}")
    ...
    # Comparison with epoch-level results
    # ------------------------------------------------------------------
    epoch_results_path = ROOT / "results" / "replay_analysis.json"
    print_comparison(summary, epoch_results_path)

    # ------------------------------------------------------------------
    # Save results
    # ------------------------------------------------------------------
    out_path = ROOT / "results" / "perwindow_replay_analysis.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    output = {
        "n_subjects": len(subjects),
        "n_controllers": len(summary),
        "pac_method": "per_window_9bins",
        "summary": summary,
