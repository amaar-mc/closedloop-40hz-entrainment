...
\vspace{0.5em}
\noindent \textbf{Keywords:} 40 Hz entrainment, phase-amplitude coupling, temporal convolutional network, closed-loop neuromodulation, Alzheimer's disease, EEG, predictive control
...
The therapeutic hypothesis underlying gamma entrainment is grounded in the disruption of normal oscillatory dynamics in AD. Gamma oscillations (30-100 Hz) are fundamental to sensory binding, attention, working memory consolidation, and inter-regional neural communication; in AD, gamma activity is reduced early in disease progression, often preceding amyloid plaque formation [2]. Theta-gamma PAC disruptions have been specifically linked to parahippocampal network hyperexcitability [3] and working memory deficits in AD patients [4] and healthy elderly [5]. This dysregulation reflects a loss of fast-spiking parvalbumin-positive (PV) interneurons that normally pace cortical gamma rhythms [6].

The mechanistic pathway from 40 Hz stimulation to amyloid clearance involves multiple processes. Sensory 40 Hz drive re-engages PV interneurons, restoring impaired gamma rhythms [6]. This triggers an immunological cascade: upregulation of cytokines promotes microglial morphological transformation and enhanced phagocytosis of amyloid plaques [11]. Additionally, Murdock et al. [7] demonstrated that multisensory 40 Hz stimulation promotes glymphatic clearance, producing a 37\% reduction in neocortical plaque volume in 5XFAD mice; pharmacological inhibition of glymphatic flow abolished the clearance effect, confirming this pathway as necessary to the therapeutic mechanism.

Translation to human subjects has progressed substantially. A 2026 study in aged rhesus monkeys found that long-term 40 Hz auditory stimulation elevated CSF amyloid-beta levels, consistent with mobilization and clearance from brain tissue [8]. In humans, Chan et al. [9] reported that patients with mild AD who received sustained 40 Hz multisensory stimulation retained strong EEG entrainment responses over time and showed less hippocampal atrophy compared to matched controls -- a finding corroborated by auditory gamma entrainment's demonstrated ability to enhance default mode network connectivity in dementia patients [10]. These results collectively establish 40 Hz entrainment as a clinically relevant therapeutic modality and make the optimization of stimulation delivery an immediate practical priority.

\subsection{Limitations of Fixed-Schedule Protocols}

Despite the therapeutic promise of 40 Hz entrainment, current clinical protocols are uniformly open-loop: they deliver stimulation according to a predetermined schedule without reference to the patient's instantaneous neural state. The standard protocol consists of alternating 40-second stimulation blocks and 20-second rest periods, repeated continuously for one hour [12, 23]. This approach is systematically misaligned with the highly variable neural dynamics of the dementia population.

The first dimension of variability is inter-individual. Sahu and Tseng [12] reported that 23 of 33 participants achieved measurable entrainment at 40 Hz while 10 showed minimal or no response -- a non-responder fraction of approximately 30\% that fixed protocols cannot detect or accommodate. High-performing patients and non-responders thus receive identical stimulation despite profoundly different neural responses.
...
\textbf{Contribution 2: Causal TCN for 5-10 second ahead PAC forecasting.} A multiscale causal Temporal Convolutional Network (31,043 parameters) trained on 73 engineered features achieves $R^2$ of approximately 0.25-0.28 at prediction horizons of 5-10 seconds, while all baseline methods including persistence and Ridge regression collapse to negative $R^2$ at these horizons -- a +0.5 $R^2$ margin for the TCN.

\textbf{Contribution 3: Closed-loop controller validated on 35 elderly subjects' EEG recordings.} The TCN-based predictive controller achieves 72.1\% alignment with patient therapeutic need versus 64.5\% for Reactive Threshold ($p < 0.001$, Hedges' $g = 1.31$), and targets 82.6\% of low-PAC windows versus 51.7\% for reactive control ($p < 0.001$, $g = 4.47$), reaching 91\% of the theoretical oracle upper bound. Every individual patient (35/35) showed improved alignment under the predictive controller.

\textbf{Contribution 4: Characterization of the prediction horizon inflection point.} Analysis across prediction horizons 1-10 seconds reveals a clear transition near 3 seconds where persistence and linear baselines cease to provide useful predictions, while the TCN maintains $R^2$ of approximately 0.25. This inflection defines the operationally critical horizon range for proactive neuromodulation.
...
The MI is computed by dividing the theta phase into $N = 18$ bins of 20 degrees each, computing the mean gamma amplitude within each phase bin, normalizing to obtain a probability distribution P(phi\_j), and evaluating the Kullback-Leibler divergence from the uniform distribution:
...
As the primary static PAC estimator, I adapted EEGNet [18] (Lawhern et al., 2018) as a regression model predicting scalar PAC from a 2-second EEG window.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Best checkpoint:} The checkpoint with lowest validation loss (epoch not recorded for EEGNet; epoch 53 refers to the TCN checkpoint).
\end{itemize}
...
I constructed a 73-dimensional causal feature vector for each 2-second window comprising three groups: (1) 61 spectral features -- band power in four frequency bands (theta, alpha, beta, gamma) across 7 channels (28 features), per-channel theta-to-gamma power ratios (7 PAC-structure features), per-channel phase-amplitude structure statistics (21 features), and 5 global statistics, computed causally via Welch periodogram; (2) 7 PAC-derived features -- current PAC, causal moving averages at 4 timescales, and first-order differences capturing PAC trajectory; (3) 5 stimulation context features -- current stimulation state, time since last state change, recent stimulation fraction, and sine/cosine cycle phase encodings. All features were z-score normalized using training-split statistics.

\subsection{Temporal PAC Forecasting: MultiscaleCausalTCN}

\subsubsection{Architecture}

\textbf{Input:} Tensors of shape (batch, T=20, F=73) -- 20 sequential 2-second windows, each with 73 features.

\textbf{Input projection:} A linear layer projects the 73-dimensional input to 64-dimensional internal representations, followed by LayerNorm and SiLU activation.
...
Best checkpoint & Epoch 53 (validation $R^2$ = 0.411) \\
Target smoothing & ts = 1 (raw PAC; no smoothing) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Target smoothing note:} Early experiments used a smoothing window of ts = 5, which inflated $R^2$ to 0.74 due to data overlap between consecutive targets. The final model uses raw (unsmoothed) targets (ts = 1) to produce honest metrics. All controller results are from the ts = 1 configuration.

\subsubsection{Performance}

On held-out test subjects, the MultiscaleCausalTCN achieved Test $R^2$ = 0.170 (raw PAC, 5-second horizon) and Test Pearson $r = 0.433$. Its practical significance emerges from the horizon sweep: at 5-10 second prediction horizons, all baseline models collapse to negative $R^2$ while the TCN maintains $R^2$ = 0.24-0.28 -- a margin of approximately +0.5 $R^2$ units.
...
\caption{Architecture of the closed-loop 40 Hz entrainment system. Raw EEG from 7 frontal channels is processed through signal processing (bandpass 0.5-80 Hz, notch, CAR), the EEGNet static PAC estimator (1,457 parameters), a 73-dimensional causal feature engineering pipeline, and the MultiscaleCausalTCN temporal forecaster (31,043 parameters, 5-second prediction horizon). The adaptive controller applies z-score thresholding against a personalized rolling baseline to determine stimulation decisions (STIMULATE / REST / MAINTAIN) with 3-second hysteresis, driving a 40 Hz auditory click train. The curved feedback arrow illustrates the closed-loop nature of the system.}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{TCN Predictive:} MultiscaleCausalTCN predicts PAC 5 seconds into the future; decisions made on the TCN's normalized delta prediction with threshold $\delta_z = 0.3$, falling back to reactive z-score ($\pm 0.5$) when insufficient TCN history is available.
...
The counterfactual nature of this evaluation means that the decisions reflect what each controller \textit{would have done} had it been deployed, but the recorded EEG reflects only the stimulation actually delivered during data collection. All results describe computed stimulation decisions and their alignment with observed PAC ground truth. Note that the 5 stimulation context features (stim state, time since switch, stim fraction, cycle phase) encode the \textit{original} fixed-schedule protocol, not the adaptive controller's hypothetical decisions; the TCN therefore has access to protocol-cycle timing that a deployed adaptive system would not share.
...
From February 5-16, 2026, I conducted a systematic exploration across eight distinct model families for static PAC prediction, ranging from compact convolutional networks to transformer-based architectures with over a million parameters. Table 3 summarizes all eight models evaluated. $R^2$ values are on the held-out test set (6 subjects, 2,822 windows).
...
\label{tab:comparison_of_eight_static_pac_predictio}
\small
...
All statistical tests are Wilcoxon signed-rank (non-parametric, paired, $N = 35$) unless otherwise noted. Effect sizes are reported as Hedges' $g$ with 95\% confidence intervals (large-sample normal approximation). All PAC values are in dimensionless Modulation Index units (Tort et al. [22]), specifically $\times 10^{-6}$ for the PAC targeting gap metric.
...
\textbf{Note on target definition:} The horizon sweep used a causal target smoothing window of ts=5 (smoothed PAC targets) to characterize comparative advantage across methods. The deployed controller checkpoint uses ts=1 (raw PAC targets) and achieves test $R^2$=0.170 at the 5-second horizon. These measure different things and should not be combined.

At short horizons, simpler methods performed competitively. At a 1-second horizon, persistence achieved $R^2$=0.760 and Ridge achieved $R^2$=0.812, while the TCN scored $R^2$=0.735. The critical transition occurred at approximately 3 seconds -- the \textbf{prediction horizon inflection point} -- where the TCN first exceeded both baselines. Beyond 3 seconds, the baselines collapsed to negative $R^2$ while the TCN maintained positive predictive accuracy.

At a 5-second horizon, persistence $R^2$=-0.267, Ridge $R^2$=-0.393, and TCN $R^2$=0.254 -- a TCN margin of +0.521 $R^2$ over persistence. At 8 and 10 seconds the pattern held, with the TCN maintaining $R^2$ of approximately 0.25. See Figure 4 for the full horizon sweep visualization.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{../../results/figures/horizon_sweep.png}
\caption{PAC forecasting performance ($R^2$) vs prediction horizon for the MultiscaleCausalTCN, persistence baseline, and Ridge baseline. Target smoothing window ts=5 for all horizons. At 1-2 second horizons, persistence and Ridge outperform the TCN. At the $\sim$3 second inflection point, the TCN begins to exceed both baselines. At 5-10 second horizons, both baselines collapse to negative $R^2$ while the TCN maintains $R^2$ of approximately 0.25 -- a +0.5 $R^2$ margin that defines the operationally actionable regime for proactive control.}
\label{fig:horizon_sweep}
\end{figure}

\textbf{Summary of horizon sweep results:}

\begin{table}[htbp]
\centering
\caption{Horizon sweep results (target smoothing ts=5). Ridge regression uses the same 73-dimensional feature vector as the TCN input.}
\label{tab:table_4}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Horizon} & \textbf{Persistence $R^2$} & \textbf{Ridge $R^2$} & \textbf{TCN $R^2$} & \textbf{TCN Margin over Persistence} \\
\midrule
...
\end{tabular}
\vspace{0.3em}
\par\small\textit{Target smoothing window ts=5 for all horizons in this sweep. Ridge regression uses the same 73-dimensional feature vector as the TCN input.}
\end{table}
...
\textbf{Controller comparison table ($N = 35$ subjects, real EEG):}

\begin{table}[htbp]
\centering
\caption{Controller comparison table ($N = 35$ subjects, real EEG):}
\label{tab:controller_comparison_table_n_35_subject}
\footnotesize
\resizebox{\textwidth}{!}{
...
For the PAC targeting gap, the TCN achieved +30.5 $\times 10^{-6}$ compared to +21.1 $\times 10^{-6}$ for reactive control ($W = 0$, $p < 0.001$; $g = +1.57$, 95\% CI [+0.98, +2.17]), a 45\% larger gap. The TCN's PAC targeting gap of 30.5 $\times 10^{-6}$ equals 91.6\% of the theoretical Alignment Oracle (33.3 $\times 10^{-6}$), referred to as 91\% in the abstract consistent with the submitted version.
...
\caption{Per-subject clinical utility comparison ($N = 35$ subjects). Each point represents one subject's clinical utility score under TCN Predictive control (y-axis) versus Reactive Threshold control (x-axis). All 35 data points fall above the y=x diagonal, confirming that every subject showed improved alignment under the predictive controller. Training subjects (filled circles), validation subjects (squares), and test subjects (triangles) are shown separately; the advantage is consistent across all three splits.}
\label{fig:per_subject_utility}
\end{figure}

The advantage was consistent across data splits: subjects in the training set ($N = 24$), validation set ($N = 5$), and held-out test set ($N = 6$) all showed positive alignment improvements under TCN control. Note that the TCN-based controllers require a 20-second warm-up period (to fill the lookback buffer) during which they default to REST; alignment metrics are computed over the full recording including this warm-up.

\subsection{Robustness Analysis}

The TCN advantage was robust across two sensitivity analyses. First, a fatigue sensitivity sweep across six habituation severity levels (rate 0.0 to 0.040) showed that the efficiency advantage of adaptive control increases monotonically with fatigue severity, from +0.4\% (no fatigue, $p=0.49$) to +5.7\% (high fatigue, $p=0.01$). Second, a threshold sensitivity sweep across z-score thresholds delta\_z=0.1 to 1.0 confirmed that the TCN outperformed the reactive baseline at all thresholds at or above delta\_z=0.2, with stable performance across delta\_z=0.3 to 1.0.

\subsection{Summary of Key Findings}

\begin{enumerate}[nosep]
  \item \textbf{Horizon inflection at $\sim$3 seconds}: Persistence and Ridge baselines outperform the TCN at 1-2 second horizons, while all baselines collapse to negative $R^2$ beyond 3 seconds where the TCN maintains $R^2$ of approximately 0.25.
  ...
  \item \textbf{Universal subject benefit}: All 35/35 subjects showed higher alignment under TCN control.
  \item \textbf{Robustness}: The TCN advantage was maintained across fatigue severity levels and across prediction confidence thresholds (delta\_z=0.2-1.0 all exceeded the reactive baseline).
\end{enumerate}
...
The closest architectural precedent is Portiloop (Lacroix et al., PLOS ONE 2022), a convolutional LSTM for real-time sleep spindle detection in EEG. However, Portiloop addresses binary classification of stereotyped waveforms, while PAC forecasting is a continuous regression problem with gradual, non-stationary transitions -- demonstrating that \textit{forecasting} rather than \textit{detection} is necessary for certain closed-loop applications. In the DBS domain, adaptive closed-loop stimulation has outperformed open-loop protocols for Parkinson's disease [20]; the present work extends this paradigm non-invasively to gamma entrainment in AD. Concurrent work has advanced related themes: Haxel et al. [16] demonstrated personalized real-time inference of cortical excitability from EEG (the PRIME framework), and ElSayed et al. [17] proposed an adaptive ML framework for auditory EEG stimulation in Fragile X Syndrome --- both validating the feasibility of real-time neural state prediction for closed-loop intervention. Broader efforts in personalized digital therapeutics [13], computational neural mass modeling of AD [14], and systematic deep learning benchmarks for EEG [15] provide additional context. Three contributions distinguish this work from these efforts: (1) PAC-specific temporal forecasting at 5-10 second horizons; (2) horizon-dependent evaluation methodology; (3) universal per-subject validation on 35 real patient EEGs.

\subsection{Limitations}

\textbf{1. Offline counterfactual replay, not live closed-loop.} The primary validation replays TCN controller decisions against recorded EEG, but cannot observe the brain's response to the controller's stimulation decisions. The 72.1\% alignment figure measures counterfactual decision quality, not realized therapeutic benefit. The counterfactual assumption that PAC dynamics would be identical under different stimulation histories is strong; real closed-loop deployment would create feedback effects not captured in offline replay. Live closed-loop validation with real-time EEG streaming and online PAC computation is required to confirm that the decision quality translates to improved therapeutic outcomes.
...
\textbf{6. Stimulation context features encode the original fixed schedule.} The 5 stimulation context features fed to the TCN during offline replay reflect the fixed-schedule protocol actually delivered during recording, not the adaptive controller's hypothetical decisions. The TCN may partly exploit the 60-second periodicity of the original protocol (via cycle phase encodings) rather than relying solely on EEG-derived predictions. A deployed adaptive system would not have access to the fixed-schedule cycle timing, potentially reducing the TCN's advantage.

\textbf{7. Primary results include training-set subjects.} The headline $N = 35$ comparison includes 24 subjects whose EEG was used to train the TCN. While the TCN predicts \textit{future} PAC (not current), and the controller comparison is conducted at the decision level rather than the prediction level, the training-set subjects' PAC distributions were seen during optimization. The per-subject plot (Figure 7) separates training, validation, and test subjects, showing the advantage is consistent across all three splits including the 6 held-out test subjects.

\textbf{8. Spectral features partially encode PAC structure.} Of the 61 spectral features, 21 are per-channel phase-amplitude structure statistics (phase resultant length, amplitude variance, preferred phase bin) that encode sub-computations of the Modulation Index. While these are computed per-window (2 seconds) and the PAC label is computed per-epoch (20-40 seconds), they represent a soft information pathway between the feature space and the target.
...
\item Deng Q, et al. Mystery of gamma wave stimulation in brain disorders. \textit{Molecular Neurodegeneration}, 19(1), 2024. \url{https://doi.org/10.1186/s13024-024-00785-x}
\item Mehak SF, et al. Theta and gamma oscillatory dynamics in mouse models of Alzheimer's disease: A path to prospective therapeutic intervention. \textit{Neuroscience and Biobehavioral Reviews}, 136, 104628, 2022. \url{https://doi.org/10.1016/j.neubiorev.2022.104628}
\item Prabhu P, et al. Abnormal gamma phase-amplitude coupling in the parahippocampal cortex is associated with network hyperexcitability in Alzheimer's disease. \textit{Brain Communications}, 6(2), fcae121, 2024. \url{https://doi.org/10.1093/braincomms/fcae121}
\item Goodman MS, et al. Theta-Gamma Coupling and Working Memory in Alzheimer's Dementia and Mild Cognitive Impairment. \textit{Frontiers in Aging Neuroscience}, 10, 101, 2018. \url{https://doi.org/10.3389/fnagi.2018.00101}
\item Abubaker M, et al. Theta-gamma-coupling as predictor of working memory performance in young and elderly healthy people. \textit{Molecular Brain}, 17(1), 2024. \url{https://doi.org/10.1186/s13041-024-01149-8}
...
\item Wang W, et al. Long-term effects of forty-hertz auditory stimulation as a treatment of Alzheimer's disease: Insights from an aged monkey model study. \textit{Proceedings of the National Academy of Sciences}, 123(2), 2026. \url{https://doi.org/10.1073/pnas.2529565123}
\item Chan D, de Weck G, et al. Gamma sensory stimulation in mild Alzheimer's dementia: An open-label extension study. \textit{Alzheimer's and Dementia}, 2025. \url{https://doi.org/10.1002/alz.70792}
\item Lahijanian M, et al. Auditory gamma-band entrainment enhances default mode network connectivity in dementia patients. \textit{Scientific Reports}, 14(1), 2024. \url{https://doi.org/10.1038/s41598-024-63727-z}
\item Garza KM, Zhang L, Borron B, Wood LB, Singer AC. Gamma Visual Stimulation Induces a Neuroimmune Signaling Profile Distinct from Acute Neuroinflammation. \textit{Journal of Neuroscience}, 40(6), 1211-1225, 2020. \url{https://doi.org/10.1523/JNEUROSCI.1511-19.2019}
\item Sahu PP, Tseng P. Gamma sensory entrainment for cognitive improvement in neurodegenerative diseases: opportunities and challenges ahead. \textit{Frontiers in Integrative Neuroscience}, 17, 1146687, 2023. \url{https://pmc.ncbi.nlm.nih.gov/articles/PMC10149720/}
\item Jiao D. Advancing personalized digital therapeutics: integrating music therapy, brainwave entrainment methods, and AI-driven biofeedback. \textit{Frontiers in Digital Health}, 7, 2025. \url{https://doi.org/10.3389/fdgth.2025.1552396}
\item Sanchez-Rodriguez LM, et al. Personalized whole-brain neural mass models reveal combined A$\beta$ and tau hyperexcitable influences in Alzheimer's disease. \textit{Communications Biology}, 7(1), 2024. \url{https://doi.org/10.1038/s42003-024-06217-2}
\item Walther D, et al. A systematic comparison of deep learning methods for EEG time series analysis. \textit{Frontiers in Neuroinformatics}, 17, 2023. \url{https://doi.org/10.3389/fninf.2023.1067095}
\item Haxel L, Ahola O, Kapoor J, Ziemann U, Macke JH. Personalized real-time inference of momentary excitability from human EEG. \textit{NeuroImage}, 322, 121547, 2025. \url{https://doi.org/10.1016/j.neuroimage.2025.121547}
\item ElSayed Z, et al. Brian Intensify: An Adaptive Machine Learning Framework for Auditory EEG Stimulation in Fragile X Syndrome. \textit{arXiv}, 2025. \url{https://arxiv.org/html/2511.09765}
...
\item Steffen S, Cannon M. Deep Learning Model Predictive Control for Deep Brain Stimulation in Parkinson's Disease. \textit{arXiv}, 2025. \url{https://arxiv.org/html/2504.00618}
\item Zanos S. Closed-Loop Neuromodulation in Physiological and Translational Research. \textit{Cold Spring Harbor Perspectives in Medicine}, 9(11), a034314, 2019. \url{https://pmc.ncbi.nlm.nih.gov/articles/PMC6824403/}
\item Soleimani G, et al. Closing the loop between brain and electrical stimulation: towards precision neuromodulation treatments. \textit{Translational Psychiatry}, 13(1), 2023. \url{https://doi.org/10.1038/s41398-023-02565-5}
...
\item Lahijanian M, et al. Non-invasive auditory brain stimulation for gamma-band entrainment in dementia patients: An EEG dataset. \textit{Data in Brief}, 40, 107763, 2022. \url{https://pmc.ncbi.nlm.nih.gov/articles/PMC8800012/}
\end{enumerate}
