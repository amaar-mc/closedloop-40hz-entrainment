...
_Practice this until it's conversational, not rehearsed:_
...
### Architecture (31,043 Parameters)

```
Input: (batch, 20 timesteps, 73 features)

Input Projection:
  Linear(73 → 64) + LayerNorm + SiLU activation
...
| Horizon | Persistence R² | Ridge R²  | TCN R²    | Who Wins?            |
| ------- | -------------- | --------- | --------- | -------------------- |
| 1s      | 0.760          | **0.812** | 0.735     | Ridge wins           |
| 2s      | 0.488          | **0.542** | 0.470     | Ridge wins           |
| 3s      | 0.234          | 0.254     | **0.277** | TCN starts winning   |
| 5s      | -0.267         | -0.393    | **0.254** | **TCN +0.52 margin** |
| 8s      | -0.276         | -0.211    | **0.240** | **TCN +0.52 margin** |
| 10s     | -0.256         | -0.212    | **0.278** | **TCN +0.53 margin** |
...
The TCN is the only method that captures the nonlinear temporal dynamics of entrainment. It maintains R-squared of 0.25 where nothing else works. That's a +0.5 margin — the difference between 'useful prediction' and 'worse than guessing.'
...
_Good luck at the Synopsys Championship on March 10!_
