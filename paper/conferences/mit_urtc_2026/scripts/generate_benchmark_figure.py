...
def generate() -> None:
    ...
    rounded_box(
        ...
        lines=[
            ...
            "No sample after t enters the input: closer to online.",
        ],
    )
    ...
    event = np.array([0.104, 0.260, 0.554])
    sliding = np.array([-0.897, 0.216, 0.212])
    ...
    ax2.set_ylim(-1.0, 0.70)
    ax2.set_ylabel("Held-out test R2", color=NAVY, fontsize=8)
    ...
    ax2.set_title("Held-out performance after target-definition stress test", fontsize=8, color=NAVY, pad=4)
