...
All converge to R² = 0.287, confirming a fundamental data ceiling (epoch-level PAC labels on 2-second windows), not a model capacity limitation. The largest model (2 million parameters) actually performed *worse* than the smallest (1,457 parameters) due to overfitting on the limited training data.
...
*Version 3 — Updated from V2: added Architecture Exploration section with 6-architecture comparison table; corrected TCN receptive field to 31-step; corrected TCN loss description to single-task Huber; updated word count estimate.*
