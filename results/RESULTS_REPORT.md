...
| Parameters | 31,043 |
| Input features | 73 (61 spectral + 7 PAC-derived + 5 stim context) |
...
**Note on R²**: Raw single-window PAC prediction from 7 frontal channels is inherently limited. The static EEGNet ceiling is R² = 0.287. At 5-second horizons, persistence and Ridge baselines collapse to negative R² (−0.27), while the TCN maintains R² ≈ 0.25 — a +0.52 margin over baselines. The TCN's value is not in absolute prediction accuracy but in directional forecasting for control decisions.
...
*Report generated from real-data replay validation on OpenNeuro ds005048. All statistics are computed from N = 35 subjects using subject-level cross-validation splits. No simulation data. Raw PAC targets (no smoothing). AI-assisted analysis disclosed.*
